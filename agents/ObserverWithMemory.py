...
class ObserverRNNAgent():
    ...
    def __init__(self, env, policy_layer_sizes, value_layer_sizes, learning_rate = 0.025):
        ...
        with tf.variable_scope('observer_agent'):                       
            ...
            self.value_scale = tf.placeholder(dtype = tf.float32, [], name = "value_scale")
            ...
            with tf.variable_scope('value'):
                ...
                self.value = tf.layers.dense(x_value, 1, activation = None)  
    ...
    def predict(self, obs_states, obs_actions, state, sess = None):
        sess = sess or tf.get_default_session()
        return sess.run(self.action_probs, {self.obs_states: obs_states,
                                            self.obs_actions: obs_actions,
                                            self.state: state})

    def update(self, obs_states, obs_actions, state, action, target, entropy_scale, sess = None):
        sess = sess or tf.get_default_session()
