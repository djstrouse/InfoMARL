...
class TabularREINFORCE:
    ...
    def __init__(self, env, use_action_info = True, use_state_info = True):
    ...
    def get_log_state_odds(self, state, goal, action, state_goal_counts, next_state, sess = None):
      sess = sess or tf.get_default_session()
      feed_dict = {self.state: state,
                   self.goal: goal,
                   self.action: action,
      ...
      return sess.run(self.log_state_odds, feed_dict)
...
# TWO PROBS: 1) nonsenical thing to plot, 2) sums over *outgoing* transitions instead of *incoming*
#def get_log_state_odds(agent, env, state_goal_counts, sess):
#  """Extracts state info array from agent."""
#  log_state_odds = np.ones((env.nS, env.nG))
#  for g in range(env.nG):
#    for s in range(env.nS):
#      # correct/incorrect goal marked by negative numbers for plotting purposes
#      if s == env.goal_locs[g]: avg_lso = -.5
#      elif s in env.goal_locs: avg_lso = -1
#      # for other states, use info estimate
#      else:
#        avg_lso = 0
#        action_probs = agent.get_action_probs(s, g, sess)
...
#            transition_prob = t[0]
#            next_s = t[1]
#            lso = agent.get_log_state_odds(s, g, a, state_goal_counts, next_s, sess)
#            avg_lso += action_prob * transition_prob * lso
#      log_state_odds[s,g] = avg_lso
#  return log_state_odds